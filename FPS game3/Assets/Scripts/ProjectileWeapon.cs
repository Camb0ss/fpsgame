...
public class ProjectileWeapon : MonoBehaviour
{
   ...
   // [SerializeField] private AudioClip sfx2; //NEW: Reload sound
    ...
    [SerializeField] private float modeDelay = 1f;
    public static bool readyToShoot = true; //make sure this is true to start
    ...
    [SerializeField] public static int maxAmmo = 30;
    public static int ammo = 30; //static so we can write our UI script easier
    ...
    private void OnReload(InputValue value) //only works if you have a button in new input named Reload
    {
       ...
       // src.Play(); //play the sound effect
        ...
        transform.Rotate(-30, 0, 0); //rotate gun down
        ammo = 0;
        Invoke(nameof(Reload), 2f);
    }

    private void OnShootMode(InputValue value)
    {
        ...
        isChanging = true;
        Invoke(nameof(shootModeSwitch), modeDelay);

    }

    private void shootModeSwitch()
    {
        if (currentShootingMode == shootingMode.autoFire)
        {
            shootingDelay = 1.5f;
            spreadIntensity = 0f;
            maxAmmo = 15;
            ...
            currentShootingMode = shootingMode.singleFire;
            modeDelay = 1f;

        }
        else if (currentShootingMode == shootingMode.singleFire)
        {
            shootingDelay = 0.25f;
            ...
            currentShootingMode = shootingMode.autoFire;
            modeDelay = 1f;
        }
        isChanging = false;
    }
    ...
    {
        isReloading = false;
        transform.Rotate(30, 0, 0); //rotate gun back into position
